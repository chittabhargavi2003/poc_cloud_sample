...
import { getAwsSuggestions } from '../api/cloudApi';
...
export default function AwsSuggestionsView() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getAwsSuggestions();
      setData(res.data);
    } catch (err) {
      ...
      <div className="flex align-items-center justify-content-between mb-4" style={{ flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <div style={{ color: '#94a3b8', fontSize: '0.85rem', marginTop: '4px' }}>
            Analyses your AWS resources, billing, and IAM configuration to detect over-used or
            under-used settings and security risks.
          </div>
        </div>
        <Button
          label="Refresh"
            ...
            >
              ...
              <div style={{ marginTop: '8px', fontSize: '0.85rem' }}>
                {(data.suggestions || []).length === 0
                  ? 'Your AWS configuration looks well-optimised!'
